...
/* =======================
   TRACE (from STATIC) localStorage only
======================= */
...
function writeTrace(list) {
  try { localStorage.setItem(TRACE_KEY, JSON.stringify(list)); } catch {}
}
function clearTrace() {
  writeTrace([]);
}
...
function renderTrace() {
  ...
  for (const t of items) {
    ...
    const msg = (typeof t.payload === "string") ? t.payload : JSON.stringify(t.payload);
...
async function onSession(session, from = "UNKNOWN") {
  ...
  // static 側が「ログイン完了」を待ってるのでイベント発火（static.js側の仕様）
  try { dispatchEvent(new Event("wired-user-ready")); } catch {}
}
